...
## 2026-05-26 — Fine-tuning pipeline: mlx-lm-lora, three-file config, Design A merge, ~/models layout

The SFT/DPO pipeline landed under `packages/orchestrator/src/orchestrator/improvement/` with a per-stage layout (`shared/`, `sft/`, `dpo/`, plus a root `merge.py`). Trainer is `mlx-lm-lora==2.1.0`; chosen over the legacy `mlx_lm.lora` because it natively supports DPO via the same YAML-driven interface, removing the need for a custom DPO loop. The pipeline runs in subprocesses (not in-process imports) so a crash in MLX cannot leave the orchestrator in a half-initialised state.

**Three-file config layout, mutually enforced.** `config/config.yaml` is the authoritative spec (random seed, generation params, training hyperparams). `config/finetuning_sft.yaml` and `config/finetuning_dpo.yaml` are the technical configs mlx-lm-lora actually reads; they MUST mirror the authoritative values. Two cross-checks run at trainer startup before any model load: `improvement.shared.preflight.preflight_check(stage)` enforces value parity (per-stage hyperparams + experiment-wide seed + SFT `model` ↔ `training.base_model`), and `improvement.shared.validate_schema.validate_schema(stage)` enforces that every key in the stage YAML appears in `mlx_lm_lora.train.CONFIG_DEFAULTS` and every enumerated value (`train_mode`, `train_type`, `dpo_cpo_loss_type`, `optimizer`, `lr_schedule.name`) is in the accepted set. Allow-lists are pulled from the installed mlx-lm-lora at import time so they track upstream automatically. Both checks together close the typo gap mlx-lm-lora itself leaves open: unknown YAML keys are silently ignored by `train.py:1187-1189`, so without the schema validator, `lr` would train at the default learning rate without anyone noticing.

**`load_in_4bits: false` everywhere — no exceptions.** Every model we load is already 4-bit on disk: the original `~/models/qwen-coder-14b-instruct-4bit/` base, the SFT-merged output, and the SFT+DPO-merged output. Setting `load_in_4bits: true` triggers `mlx_lm_lora.utils:211` to call `nn.quantize` on an already-quantized model, which raises `ValueError("Cannot quantize already quantized model")` and aborts the run. The flag is for the standard QLoRA flow (load fp16, quantize on load); we don't use that flow because the base ships pre-quantized and our merge step (see Design A below) deliberately re-quantizes the SFT-merged checkpoint at the same `{bits: 4, group_size: 64, mode: affine}` shape, so DPO loads it the same way SFT loads the base.

**Design A merge: 4-bit only, no persistent fp16 staging.** `improvement.merge.merge_and_quantize` runs `mlx_lm fuse --dequantize` to merge bf16 LoRA into the (possibly 4-bit) base and dequantize to fp16 inside a `TemporaryDirectory`, then `mlx_lm convert -q --q-bits 4 --q-group-size 64 --q-mode affine` to re-quantize that fp16 into the final 4-bit checkpoint. The fp16 artifact lives only inside the TemporaryDirectory and is deleted when the merge exits. Quantization shape is read from the base's `config.json` so the output is byte-compatible with whatever loaded it. Considered and rejected Design B (keep `sft-merged-fp16/` persistent and have DPO QLoRA-quantize on load) because it doubles disk per stage (+28 GB), requires running mlx-lm-lora's quantizer at every DPO start, and makes determinism depend on the quantizer being stable across versions.

**Merged checkpoints live under `~/models/`, not in the repo.** The base is at `~/models/qwen-coder-14b-instruct-4bit/`; the SFT output goes to `~/models/qwen-coder-14b-instruct-4bit-tried-sft/` and the SFT+DPO output to `~/models/qwen-coder-14b-instruct-4bit-tried-sft-dpo/`. These are loadable MLX checkpoints and belong with the base, not in the repo. LoRA adapter dirs remain under `data/improvement/checkpoints/` because they are training intermediates the merge step consumes and discards.

**Base model corrected to the Instruct variant.** Initial config used `mlx-community/Qwen2.5-Coder-14B-4bit` (the non-Instruct completion model). Eval and SFT data both rely on `apply_chat_template` from `Qwen/Qwen2.5-Coder-14B-Instruct`, so training the non-Instruct base on chat-formatted data would teach the model the template, not Triton. The base is now `~/models/qwen-coder-14b-instruct-4bit/` (Instruct variant, downloaded by `scripts/download_mlx_base_model.sh`).

**~/ in YAMLs.** Path fields in `config.yaml` and the stage YAMLs use `~/...` for repo portability. mlx-lm does NOT expanduser path fields (`mlx_lm.utils.get_model_path` calls `Path(...).exists()` directly), so each trainer wrapper materialises a `_resolved_config.yaml` next to the stage's data dir with `~/` expanded before subprocessing mlx-lm-lora. The resolved file is kept on disk for post-mortem inspection.

**`shared.logging` auto-loads packages/{orchestrator,verification}/.env.** Previously, `load_dotenv()` walking up from the repo root never found the orchestrator's `.env` (it lives one level deeper than the walk-up looks). Every entry point that needed `TRIED_ROLE` or API keys depended on the operator having them exported in their shell. The shared logger now auto-discovers and loads the role's `.env` at import time with `override=False`, so existing shell-exported values still take precedence. python-dotenv is a soft import — if it's not installed (verification machine), the discovery is skipped and the role falls back to "unknown".

**Layer-2 smoke test.** `packages/tests/src/tried_tests/finetuning_smoke.py` builds a 2-row synthetic SFT dataset and runs `mlx_lm_lora.train` with smoke-mode overrides (iters=2, batch=1, max_seq=256, lr_schedule=None) against the real base. Catches mlx-lm-lora YAML acceptance regressions, the "Cannot quantize already quantized model" class of crash, chat data-format bugs, and adapter-save bugs. Runs end-to-end in ~17s on the M4 (`uv run python -m tried_tests.finetuning_smoke`).

Files: `packages/orchestrator/src/orchestrator/improvement/{__init__.py,merge.py,shared/*,sft/*,dpo/*}`, `config/{config.yaml,finetuning_sft.yaml,finetuning_dpo.yaml}`, `packages/tests/`, `packages/shared/src/shared/logging/__init__.py`, `packages/orchestrator/pyproject.toml` (mlx-lm-lora dep), `pyproject.toml` (tried-tests workspace dep), `scripts/download_mlx_base_model.sh`.
